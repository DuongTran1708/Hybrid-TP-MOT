...
from thermal_pedestrian.core.factory.builder import CAMERAS, DETECTORS, TRACKERS, IDENTIFICATIONS
...
from thermal_pedestrian.matchers.tracklets import is_matching_tracklets
from thermal_pedestrian.reidentifications import BaseReID_Adapter
from thermal_pedestrian.trackers.basetracker_adapter import BaseTracker_Adapter
from thermal_pedestrian.core.objects.instance import Instance
from thermal_pedestrian.core.utils.bbox import bbox_xyxy_to_xywh, bbox_cxcywh_norm_to_xyxy, bbox_cxcywh_norm_to_xywh
...
class ThermalCamera(BaseCamera):
	...
	def __init__(
			...
			name          : str,
			detector      : dict,
			detector_reidentification      : dict,
			tracker       : dict,
			tracker_reidentification       : dict,
			heuristic     : dict,
			data_loader   : dict,
		...
		Args:
			...
			tracker (BaseTracker_Adapter, dict):
				Tracking object or a tracker's config dictionary.
			heuristic (HeuristicModule, dict):
		...
		self.detector_reid_cfg = detector_reidentification
		self.tracker_cfg       = tracker
		self.tracker_reid_cfg  = tracker_reidentification
	...
	def init_tracker(self, tracker: Union[BaseTracker_Adapter, dict]):
		"""Initialize tracker.
		...
		self.tracker = None
		if isinstance(tracker, BaseTracker_Adapter):
	...
	def init_reidentification(self, identification_cfg: dict):
		"""Initialize re-identification.

		Args:
			identification (object, dict):
				Identification object or a identification's config dictionary.
		"""
		self.identification = []
		for reid_model in identification_cfg["reid_model"]:
			console.log(f"Initiate {reid_model['model']['name']}.")
			if isinstance(reid_model['model'], dict):
				self.identification.append(IDENTIFICATIONS.build(**reid_model['model']))
			else:
				raise ValueError(f"Cannot initialize identification with {reid_model['model']['name']}.")
				...
				for index_batch, (index_image, file_path_img, batch) in enumerate(zip(indexes, files_path, batch_instances)):
					...
					with open(file_path_txt_ou, 'w') as f_write:
						for index_in, instance in enumerate(batch):
							...
							f_write.write(f"{class_id} {bbox_xyxyn[0]} {bbox_xyxyn[1]} {bbox_xyxyn[2]} {bbox_xyxyn[3]} {score}\n")
	...
	def run_detector_reidentification(self):
		"""Run detection and reidentification model"""
		...
		file_reid_pkl_ou = os.path.join(
			self.data_writer_cfg['output_dir'],
			"detection",
			self.detector_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
			"thermal",
			f"{self.data_writer_cfg['seq_cur']}_reid.pkl"
		)
		os.makedirs(os.path.dirname(file_reid_pkl_ou), exist_ok=True)
		...
		# run tracking
		detector_reid_pkl = {}
		for img_index, img_name in enumerate(tqdm(list_imgs, desc=f"Detector Re-Identification: {self.data_writer_cfg['seq_cur']}")):
			# init
			...
			with open(det_path, "r") as f_read:
				for line in f_read:
					...
					bbox         = bbox_cxcywh_norm_to_xywh(np.array(det[1: 5], dtype=np.float32), float(img.shape[0]), float(img.shape[1]))
					x1, y1, w, h = bbox[0], bbox[1], bbox[2], bbox[3]
					confidence   = det[5],
					class_label  = self.class_labels.class_labels[det[0]]
					reid = []
					if len(self.identification) > 0:
						reid         = [self.identification[i].extract_feature(
										img,
										np.array([[x1, y1, w, h]])
									) for i in range(len(self.identification))]
					bboxes.append({
						"class_id"  : det[0],
			...
			detector_reid_pkl[base_name] = bboxes

		# Output tracklets with pickle
		with open(file_reid_pkl_ou, "wb") as f_write:
			pickle.dump(detector_reid_pkl, f_write)

	def run_tracker_forward(self):
		"""Run tracking model"""
		# input folder directory to load detection
		folder_det_pkl_in = os.path.join(
			self.data_writer_cfg['output_dir'],
			"detection",
			self.detector_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
		...
		# load detection with pickle
		with open(folder_det_pkl_in, "rb") as f_read:
			detector_reid_pkl = pickle.load(f_read)
			# detector_reid_pkl      = dict(sorted(detector_reid_pkl_temp.items()))

		with open(file_mot_ou, 'w') as f_write:
			# run tracking
			for img_index, (base_name, bboxes_info) in enumerate(tqdm(detector_reid_pkl.items(), desc=f"Tracking forward: {self.data_writer_cfg['seq_cur']}")):
				# init
				img_path = os.path.join(self.data_loader_cfg['data_path'], f"{base_name}.png")
				...
				for bbox_info in bboxes_info:
					instance = Instance(
						video_name   = self.data_writer_cfg['seq_cur'],
						frame_index  = img_index,
						bbox         = np.array(bbox_info["bbox"]), # xyxy
						confidence   = bbox_info["confidence"],
						class_label  = self.class_labels.class_labels[bbox_info["class_id"]],
						feature      = bbox_info["reid"][0][0] # to get (n,) from (1,..., n)
					)

					# DEBUG:
					# print("*" * 50)
					# print(bbox_info["reid"][0][0].shape)  # (768,)
					# print("*" * 50)
					# import sys
					# sys.exit()
				...
				for gmo in gmos:
					if gmo.time_since_update > 0:  # the track does not have any dets match
						continue
					bbox = bbox_xyxy_to_xywh(gmo.current_bbox) * [1, 1, 0.86, 0.86]
	...
	def run_post_reidentification(self):
		"""Run reidentification"""
		# NOTE: init parameter
		# Get tracking result
		file_mot_in = os.path.join(
			self.data_writer_cfg['output_dir'],
			"tracking",
			self.tracker_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
		...
		# create file to store result with pickle
		file_mot_pkl_ou = os.path.join(
			self.data_writer_cfg['output_dir'],
			"tracking",
			self.tracker_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
			"thermal",
			f"{self.data_writer_cfg['seq_cur']}_thermal_reid.pkl"
		)
		os.makedirs(os.path.dirname(file_mot_pkl_ou), exist_ok=True)

		# LOAD TRACKLETS
		tracklets = {}
		with open(file_mot_in, "r") as f_read:
			...
			img_index_current = 0
			for index_line, line in enumerate(tqdm(lines, desc=f"{self.data_writer_cfg['seq_cur']}")):
				# '{frame},{id},{x1},{y1},{w},{h},{s},{label},-1,-1\n'
				line = line.strip().split(",")
				...
				if frame_index != img_index_current or img is None:
					img_index_current = frame_index
					img = cv2.imread(os.path.join(self.data_loader_cfg['data_path'], list_imgs[img_index_current - 1])) # PBVS rule, frame start from 1
				...
				tracklets[track_id]["frames"].append(frame_index)
				tracklets[track_id]["bboxes"].append([x1, y1, w, h])
				tracklets[track_id]["scores"].append(score)
				tracklets[track_id]["frame_end"] = frame_index  # PBVS rule, frame start from 1
				tracklets[track_id]["feats"].append({
					"reid" : [self.identification[i].extract_feature(
								img,
								np.array([[x1, y1, w, h, track_id]])
							) for i in range(len(self.identification))]
				})

		# Output tracklets with pickle
		with open(file_mot_pkl_ou, "wb") as f_write:
			pickle.dump(tracklets, f_write)

	def run_heuristic(self):
		"""Run heuristic model"""
		# create file to store result with pickle
		file_mot_pkl_in = os.path.join(
			self.data_writer_cfg['output_dir'],
			"tracking",
			self.tracker_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
		...
		# output result file after reid and heuristic
		file_mot_after_reid_ou = os.path.join(
			self.data_writer_cfg['output_dir'],
			"tracking",
			self.tracker_cfg['folder_out'],
			self.data_writer_cfg['seq_cur'],
			"thermal",
			f"{self.data_writer_cfg['seq_cur']}_thermal_after_reid.txt"
		)

		# read pickle file
		with open(file_mot_pkl_in, "rb") as f_read:
			tracklets = pickle.load(f_read)
		# sort tracklets by key (track_id)
		tracklets = dict(sorted(deepcopy(tracklets).items()))

		# count total tracklets before heuristic:
		print(f"\n{self.data_writer_cfg['seq_cur']} - Total tracklets before: {len(tracklets)}")

		# SUGAR:
		# NOTE: FILTER tracklets by length
		# tracklets = {key: value for key, value in tracklets.items() if len(value["frames"]) >= self.heuristic_cfg["min_length_tracklet"]}

		# run heuristic when dict not empty, meaning have tracklets not processed
		tracklets_result = {}
		while len(tracklets) > 0:
			tracklet_id   = next(iter(tracklets))       # get first key
			tracklet_data = tracklets.pop(tracklet_id)  # get and remove by key

			# list of tracklet match with current tracklet
			list_tracklet_id_match = []

			# match remain tracklets to current tracklet
			for tracklet_id_remain, tracklet_data_remain in tracklets.items():
				# run matching tracklets
				is_match = is_matching_tracklets(
					tracklet_data,
					tracklet_data_remain,
					self.heuristic_cfg
				)

				# DEBUG:
				# if self.data_writer_cfg['seq_cur'] == "seq47":
				# 	if tracklet_id_remain == 21 and tracklet_id == 2:
				# 		is_match = True

				# if match, merge two tracklets
				if is_match:
					# merge tracklet_data_remain to tracklet_data
					tracklet_data["frames"].extend(tracklet_data_remain["frames"])
					tracklet_data["bboxes"].extend(tracklet_data_remain["bboxes"])
					tracklet_data["scores"].extend(tracklet_data_remain["scores"])
					tracklet_data["feats"].extend(tracklet_data_remain["feats"])
					tracklet_data["frame_start"] = min(tracklet_data["frame_start"], tracklet_data_remain["frame_start"])
					tracklet_data["frame_end"]   = max(tracklet_data["frame_end"],   tracklet_data_remain["frame_end"])

					# add to remove list
					list_tracklet_id_match.append(tracklet_id_remain)

			# remove matched tracklets
			for tracklet_id_match in list_tracklet_id_match:
				tracklets.pop(tracklet_id_match)

			tracklets_result[tracklet_id] = tracklet_data

		# count total tracklets after heuristic:
		print(f"{self.data_writer_cfg['seq_cur']} - Total tracklets after: {len(tracklets_result)}")

		# Convert tracklets Dict to MOT Array format
		# '{frame},{id},{x1},{y1},{w},{h},{s},{label},-1,-1\n'
		arr_mot_result = []
		for key, value in tracklets_result.items():
			tracklet_id   = key
			tracklet_data = value
			frames = tracklet_data["frames"]
			bboxes = tracklet_data["bboxes"]
			scores = tracklet_data["scores"]
			label_id = tracklet_data["label_id"]
			for bbox, frame_index, score in zip(bboxes, frames, scores):
				arr_mot_result.append([
					frame_index,
					tracklet_id,
					abs(bbox[0]),
					abs(bbox[1]),
		...
		arr_mot_result = sorted(arr_mot_result, key=itemgetter(0, 1))
		# write to file
		with open(file_mot_after_reid_ou, "w") as f_write:
			for result in arr_mot_result:
				# '{frame},{id},{x1},{y1},{w},{h},{s},{label},-1,-1\n'
		...
		if self.drawing:
			folder_img_ou = os.path.join(
				...
				self.tracker_cfg['folder_out'],
				self.data_writer_cfg['seq_cur'],
				"thermal/img_draw_reid"
			)
			os.makedirs(folder_img_ou, exist_ok=True)
			# load list images FORWARDING
			list_imgs = [s for s in sorted(os.listdir(self.data_loader_cfg['data_path']), key=cmp_to_key(compare_filenames), reverse=False)]

			img_current_index = None
			img               = None
			for result in tqdm(arr_mot_result, desc=f"Drawing reid : {self.data_writer_cfg['seq_cur']}"):

				if img_current_index is None or img_current_index != result[0]:
					if img_current_index is not None and img is not None:
						# save image
						cv2.imwrite(os.path.join(folder_img_ou, list_imgs[img_current_index - 1]), img)

					img_current_index = result[0]
					img_path          = os.path.join(self.data_loader_cfg['data_path'], list_imgs[img_current_index - 1]) # PBVS rule, frame start from 1
					img               = cv2.imread(img_path)

				img = plot_one_box_voc(
					bbox  = [result[2], result[3], result[2]+result[4], result[3]+result[5]],
					img   = img,
					color = create_unique_color_uchar(int(result[1]) % 1000),
					label = f"{int(result[1]) % 1000}"
				)
	...
	def run(self):
		...
		for seq in tqdm(self.data_loader_cfg.data_dir_seq):
			...
			# NOTE: Detection Re-identification process
			if self.process["function_detection_reid"]:
				self.init_reidentification(identification_cfg=self.detector_reid_cfg)
				self.run_detector_reidentification()
			...
			# NOTE: Tracking Re-identification process
			if self.process["function_tracking_reid"]:
				self.init_reidentification(identification_cfg=self.tracker_reid_cfg)
				self.run_post_reidentification()
				if hasattr(self, "identification"):
					del self.identification
					self.identification = None

			# NOTE: Post heuristic process
...
def plot_one_box_yolo(bbox, img, color=None, label=None, line_thickness=1):
	...
	"""
	x_min, y_min, x_max, y_max =  bbox_cxcywh_norm_to_xyxy(bbox, img.shape[0], img.shape[1])
